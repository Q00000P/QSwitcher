...
// === Логирование в файл ===
// Все print() и fputs(stderr) дублируются в ~/Library/Logs/AutoSwitcher.log
// чтобы можно было разобраться с проблемой постфактум, без запуска из терминала.
func setupFileLogging() {
    let logsDir = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask)[0]
        .appendingPathComponent("Logs", isDirectory: true)
    try? FileManager.default.createDirectory(at: logsDir, withIntermediateDirectories: true)
    let logFile = logsDir.appendingPathComponent("AutoSwitcher.log")
    ...
    if let attrs = try? FileManager.default.attributesOfItem(atPath: logFile.path),
       let size = attrs[.size] as? Int, size > 5_000_000 {
        let old = logsDir.appendingPathComponent("AutoSwitcher.log.old")
